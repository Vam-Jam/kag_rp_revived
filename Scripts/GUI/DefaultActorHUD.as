...
{//462,40
	GUI::DrawText("[Place Holder]", origin+Vec2f(565,8), SColor(255,50,200,200));
	GUI::DrawIcon("EXP_Temp.png", 0, Vec2f(1000,13), origin+Vec2f(500,25));
	//GUI::DrawPane(origin + Vec2f(462,6),origin+Vec2f(600,42) , SColor(200,50,50,50	));
}
...
void onRender(CSprite@ this)
{
	...
	{
		ul = Vec2f(HUD_X - dim.x / 1.2f, HUD_Y - dim.y + 12);
	}
	else
	{
		ul = Vec2f(HUD_X - dim.x / 2.0f, HUD_Y - dim.y + 12);
	}
	Vec2f lr(ul.x + dim.x, ul.y + dim.y);
	//GUI::DrawPane(ul, lr);
	renderBackBar(ul, dim.x + 50, 1.0f);
	...
	renderHPBar(blob, ul);
	renderFrontStone(ul+Vec2f(700,0), 200, 1.0f);
	renderEXPBar(blob, ul);
