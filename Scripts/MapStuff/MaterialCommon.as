...
namespace Material
{
  ...
  void fromTile(CBlob@ this, uint16 &in type, float &in damage)
  {
    ...
    if (!isBlockSolidTT(map, type)) return;
	
	//print("away: "+type);
	
	BlockInfo@ Block = getBlockTT(map, type);
	
	if(Block !is null)
	{
		if(Block.name == "hard stone ore")
			createFor(this, 'mat_stone', 6.f * damage);

		if(Block.name == "stone ore")
			createFor(this, 'mat_stone', 4.f * damage);

		if(Block.name == "stone wall")
			createFor(this, 'mat_stone', damage);

		if(Block.name == "wood wall")
			createFor(this, 'mat_stone', damage);

		if(Block.name == "gold ore")
			createFor(this, 'mat_stone', 4.f * damage);
	}
