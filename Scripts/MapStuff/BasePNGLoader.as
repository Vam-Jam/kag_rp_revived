...
class PNGLoader
{
	...
	void handleOffset(int type, int offset, int position, int count)
	{
		...
		case tree_offset:
		{
			...
			if(tree !is null)
			{
				...
				{
					map.SetTile(offset, CMap::tile_newgrass + map_random.NextRanged(4) );
				}
			}
		}
		break;
	...
	void SetupMap(int width, int height)
	{
		...
		map.legacyTileVariations = map.legacyTileEffects = map.legacyTileDestroy = false;
	}
