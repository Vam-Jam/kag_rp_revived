	...
	if (map.getHitInfosFromArc(pos, aimangle, arcdegrees, radius + attack_distance, this, @hitInfos))
	{
		...
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			if (b !is null && !dontHitMore) // blob
			{
				...
				{
					BlockInfo@ Block = getBlockTT(map, hi.tile);
					bool ground = Block.name == "dirt";
					bool dirt_stone = Block.name == "hard stone ore" || Block.name == "stone ore" || Block.name == "copper ore";
					bool gold = Block.name == "gold ore";
					bool wood = Block.name == "wood wall";
					bool easy = Block.name == "vine" || Block.name == "charred wood wall" || Block.name == "rubble";
					if (ground || wood || dirt_stone || gold || easy)
					{
	        ...
	        (deltaInt == DELTA_BEGIN_ATTACK + 1)) // hit only once
	{
		...
		for (int y = 0; y < steps; y++)
			for (int x = 0; x < steps; x++)
			{
				...
				if (getBlockTT(map, tile).name == "grass")
				{
