...
const string orcHeads = CurrentModName+"/Entities/PlayableChars/Sprites/Heads/OrcHeads.png";
const string dwarfHeads = CurrentModName+"/Entities/PlayableChars/Sprites/Heads/DwarfHeads.png";
const string elfHeads = "";
...
CSpriteLayer@ LoadHead(CSprite@ this, int headIndex)
{
	...
	// add head
	int dlc_number = get_dlc_number(headIndex);//returns 0 if non DLC
	...
	{
		case 0:
			if(rules.get_string("Team0") == "Humans")
			{
		...
		case 1: 
			if(rules.get_string("Team1") == "Dwarfs")
			{
				@head = this.addSpriteLayer("head", dwarfHeads, 16, 16,
		        		(dlc.do_teamcolour ? this.getBlob().getTeamNum() : 0),
		                (dlc.do_skincolour ? this.getBlob().getSkinNum() : 0));
			}
			break;
		...
		case 2:
			if(rules.get_string("Team2") == "Elves")
			{
				@head = this.addSpriteLayer("head", elfHeads, 16, 16,
		        		(dlc.do_teamcolour ? this.getBlob().getTeamNum() : 0),
		                (dlc.do_skincolour ? this.getBlob().getSkinNum() : 0));
			}
			break;
		...
		case 3:
			if(rules.get_string("Team3") == "Orcs")
			{
				@head = this.addSpriteLayer("head", orcHeads, 16, 16,
		        		(dlc.do_teamcolour ? this.getBlob().getTeamNum() : 0),
		                (dlc.do_skincolour ? this.getBlob().getSkinNum() : 0));
			}
			break;
