...
bool onServerProcessChat(CRules@ this, const string& in text_in, string& out text_out, CPlayer@ player)
{
	...
	if (sv_test)
	{
		...
		else if(text_in == "!rank")
		{
			print(""+this.get_u8(getTeamNumber(0,1)));
		}
		...
		{
			print(""+this.get_u32(player.getUsername() + " EXP " + player.getBlob().getName()));
		}
		...
		else if (text_in.substr(0, 1) == "!")
		{
			...
			if (tokens.length > 1)
			{
				...
				{
					int status = parseInt(tokens[1]);
					syncDiplomatStatus(this,0,1,status);
				}
				return true;
			}
